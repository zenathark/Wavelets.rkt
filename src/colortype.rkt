#lang racket/base

(provide AbstractRGB
         RGB
         RGB-red
         RGB-green
         RGB-blue
         AbstractGray
         Gray)

;;---------------
;;-----Binary RGB Colors
(struct AbstractRGB ())
(struct AbstractBinaryRGB AbstractRGB ())
;; 8bit per channel RGB
(struct RGB8 AbstractBinaryRGB (red green blue [size #:auto])
  #:auto-value 8)
(struct BGR8 AbstractBinaryRGB (blue green red [size #:auto])
  #:auto-value 8)
;; 16bit per channel RGB
(struct RGB16 AbstractBinaryRGB (red green blue [size #:auto])
  #:auto-value 16)
(struct BGR16 AbstractBinaryRGB (blue green red [size #:auto])
  #:auto-value 16)

(struct RGB AbstractRGB (red green blue))
(struct AbstractGray ())
(struct Gray AbstractGray (gray))
(struct HSV (hue saturation value))

;; (define-generic (red color))
;; (define-instance ((red RGB8) color)
;;   (RGB8-red color))
;; (define-instance ((red RGB16) color)
;;   (RGB16-red color))

;;---------------
;;-----Gray Color
;; (struct AbstractGray ())
;; ;; 8bit channel gray
;; (struct Gray8 AbstractGray (gray))
;; ;; 16bit channel gray
;; (struct Gray16 AbstractGray (gray))
;; ;; Alias to 8bit Gray
;; (struct Gray Gray8 (gray))
